...
- ODS层：Operation Data Store，直接从业务系统采集到的最原始的数据，属于操作数据层。
- DWD层：Data Warehouse Detail，在ODS层上经过业务过程建模产出的实时事实明细层。
- DWS层：Data WareHouse Servce，订阅明细层数据后，会在实时任务中计算各维度汇总指标。如果维度时比较通用的，会放在DWS层。
