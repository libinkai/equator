...
> 如何有效地降低存储资源的消耗，节省存储成本，是存储管理孜孜以求的目标。
...
- MaxCompute基于列式储存，由于每个表数据分布不同，会导致插入数据的顺序不一样，导致压缩效果差异大。可以通过`distribute by`与`sort by`方法进行数据重分布。
...
- 对存储、计算、扫描三者进行计量之后，对其进行计费操作。

## 题外话

- 在数据量不大的情况下，很多公司都不太会进行存储和成本的考虑，保证业务快速增长就行了。但是当一个产品增长放缓时，对各项进行治理优化，降低成本的操作将被提上日程。有些DAU高的APP，其一次大规模的治理活动，可能会带来每年数千万元的成本节约，正所谓开源节流啊。